...
def train(rank=None, args=None):
    ...
    dataloader_img_val, dataloader_lbl_val , dataloader_sam_val = get_dataset(args=args, dataset_path=args.dataset_source_path, is_train=False, is_val=True, distributed=False)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, (list_img_src, list_lbl_src, list_img_trg, list_sam_src, list_sam_trg) in enumerate(zip(pbar_img, dataloader_lbl_src, dataloader_img_trg, dataloader_sam_src, dataloader_sam_trg)):
            ...
            with autocast(enabled=amp):
                ...
                concat_source = torch.cat([x_time_source, images_src, sam_src], dim=1) #Should be of size (B,9,H,W) (img+noise+pseudo_lbl)
                predicted_noise_source = model.forward(concat_source, time)
                loss_source = mse(noise_soure, predicted_noise_source)

                #concat_target = torch.cat([x_time_target, images_trg, sam_trg], dim=1) #Should be of size (B,9,H,W) (img+noise+pseudo_lbl)
                #predicted_noise_target = model.forward(concat_target, time)
                #loss_target = mse(noise_target, predicted_noise_target)

                loss_diff = loss_source
            ...
            src_loss_diff_list.append(loss_source.detach().cpu().numpy().item())
            #trg_loss_diff_list.append(loss_target.detach().cpu().numpy().item())
        # Loss per epoch
        ...
        if save_models:
            ...
            if vis:
                ...
                # Plotting the loss
                plt.plot(iteration, src_loss_diff_list, label='source Loss')
                #plt.plot(iteration, trg_loss_diff_list, label='target Loss')
